...
export const sendResetEmail = async (email: string, token: string) => {
  ...
  const mailOptions = {
    from: `"Mayamoy Builders" <${process.env.EMAIL_USER}>`,
    to: email,
    subject: 'Password Reset Request - Mayamoy Builders',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
        <h2 style="color: #0d9488; text-align: center;">Mayamoy Builders</h2>
        <p>Hello,</p>
        ...
        <p style="font-size: 12px; color: #888;">Mayamoy Builders - Your Trusted Online Store</p>
      </div>
    `,
  };
